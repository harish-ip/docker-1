...
@Component({
  selector: 'app-client-notifications-list',
  templateUrl: './client-notifications-list.component.html',
  styleUrls: ['./client-notifications-list.component.css']
})
export class ClientNotificationsListComponent implements OnInit {
  searchTerm: string;
  ...
  openClientNotification(notificationId) {
    console.log('notiid is' + notificationId);
    this.reg.clientOpenNotification(notificationId).subscribe(
      (Response) => {
        console.log(Response);
        this.reg.notification = Response;
        this.reg.clickClientNotication.next(true);
        this.router.navigate(['/clientadmin/notifications/open-client-notification']);
      },
      (err) => {
        console.log(err);
      }
