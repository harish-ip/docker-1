...
export class CreateProductComponent implements OnInit {
  ...
  productValidations(): boolean {
    if (this.ImageFileName !== null) {
      ...
      const test1 = (/\.(gif|jpg|jpeg|png)$/i).test(this.ImageFileName);
      console.log(test1);
      if (!test1) {
  ...
  processFile(imageInput: any) {
    ...
    if (imageInput.target.files[0].size < 250000) {
  ...
  createProducts() {
    ...
    if (this.productValidations() && this.validateProductDuration(this.product.productDuration) && this.prodValidation()) {
