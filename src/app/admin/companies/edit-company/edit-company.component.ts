...
export class EditCompanyComponent implements OnInit {
  ...
  changeStateCity() {
    let registerData: Register = JSON.parse(sessionStorage.getItem('CompanyDetails'));
  ...
  isExisted() {
    this.registerservice.isUserExist(this.register.email).subscribe(() => {
  ...
  updateRegisterCompanies() {
    ...
    this.service.editRegisterTenant(this.register).subscribe(
      (res) => {
        ...
        this.router.navigate(['/admin/companies/companies-list']);
      },
      (err) => {
        console.log(err);
        this.loading = false;
